...
- Serial : 단일 스레드에서만 작업을 처리한다.
- Concurrent : 다중 스레드에서 작업을 처리한다.
...
#### DispatchSemaphore에서 궁금했던 점

- value 값을 두개 주고 실행했을 때 
...
🔥🔥 조건문을 넣지 않을 경우 하나의 비동기 코드블럭에서 여러 스레드가 하나의 공유 자원에 동시에 접근하기 때문에 같은 값을 다루는 오류가 발생한다. `if`문과 같은 조건문을 걸어서 값에 접근하는 것에 약간의 텀을 두어서 이를 해결할 수 있는것으로 추측된다...
